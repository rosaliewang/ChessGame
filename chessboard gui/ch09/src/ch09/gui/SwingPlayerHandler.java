...
import ch09.logic.MoveValidator;
 ...
 * 
 */
public class SwingPlayerHandler extends JPanel implements IPlayerHandler{
	 ...
	 */
	public SwingPlayerHandler(ChessGame chessGame) {
		this.setLayout(null);

		// background
		URL urlBackgroundImg = getClass().getResource("/ch09/gui/img/bo.png");
	...
	private Image getImageForPiece(int color, int type) {
		...
		URL urlPieceImg = getClass().getResource("/ch09/gui/img/" + filename);
	...
	public void setNewPieceLocation(GuiPiece dragPiece, int x, int y) {
		int targetRow = SwingPlayerHandler.convertYToRow(y);
		int targetColumn = SwingPlayerHandler.convertXToColumn(x);
	...
	@Override
	public Move getMove() {
		// enable dragging of game pieces, as a call to this method indicates,
		// that it is our turn to move.
		this.draggingGamePiecesEnabled = true;
		
		// return current move and reset it to null
	...
	public static void main(String[] args) {
		...
		SwingPlayerHandler chessGui = new SwingPlayerHandler(chessGame);
